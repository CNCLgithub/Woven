...
NvFlexTriangleMeshId CreateTriangleMesh(Mesh* m, bool render=false)
{
    if (!m)
        return 0;
...
void CreateSpringGrid_rotate(Vec3 lower, int dx, int dy, int dz, float radius, int phase, float stretchStiffness, float bendStiffness, float shearStiffness, Vec3 velocity, float invMass, float extra_spacing=0.0f, float extra_rad_mult=1.0f)
{
    int baseIndex = int(g_buffers->positions.size());
    ...
    for (int x=0; x < dx; ++x)
    {
        for (int y=0; y < dy; ++y)
        {
            for (int z=0; z < dz; ++z)
            {
                ...
                if (z > 0 && y > 0)
                {
                    g_buffers->triangles.push_back(baseIndex + GridIndex(z-1, y-1, dz));
                    g_buffers->triangles.push_back(baseIndex + GridIndex(z, y-1, dz));
                    g_buffers->triangles.push_back(baseIndex + GridIndex(z, y, dz));
                    
                    g_buffers->triangles.push_back(baseIndex + GridIndex(z-1, y-1, dz));
                    g_buffers->triangles.push_back(baseIndex + GridIndex(z, y, dz));
                    g_buffers->triangles.push_back(baseIndex + GridIndex(z-1, y, dz));
