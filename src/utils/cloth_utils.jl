...
function filter_flatten_cloth(cloth_pos::Array{Array{Float64, 1}, 1},
    ...
    for i in total_edge_layers:total_points - (total_edge_layers + 1)  # rows
        if i % 4 != 0  # select alternate rows
            for j in total_edge_layers + 1:total_points - total_edge_layers  # columns
                append!(delete_indices, total_points * i + j)
            end
            continue
        end
        for j in total_edge_layers + 1:total_points - total_edge_layers  # columns
            if j % 4 == 0  # select alternate rows
                continue
            end
            append!(delete_indices, total_points * i + j)
        end
